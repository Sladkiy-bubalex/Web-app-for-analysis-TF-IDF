...
- Получение доменного имени для сайта:
  - Если вы не хотите зарегистрировать доменное имя для сайта, то в файле ```nginx.conf``` в разделе ```server_name``` укажите IP-адрес в формате ```server_name your_ip_address;```
  - Если вы хотите, то в файле ```nginx.conf``` в соответствующем разделе укажите доменное имя
